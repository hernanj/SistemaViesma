...
';
   ...
   function close_totals(){
    // inicio de variables para locales 
	$total_productos=0;
	$cant_ventas=0;
	$acumlado=0;
	$aux_salida='';	
	// Variables para cierre de dia
	$details_sales=array();
	$total_sales = array();
	$products_totals=array();
	
	// Recuperamos las ventas del dia 
	$ventas=$this->reports_model->getTotalSalesDaily();	
	 //Traemmos cantidad de productos vendidos
	$productos=$this->reports_model->getProductsSalesDaily();
	
	if(!empty($ventas)){	
		// totales detallados de ventas
		
		$aux_salida="	   
		<h3>Ventas al día de hoy </h3>	
		   <table width='100%'>
				<tr>
					<td valign='top'>
					<h4>Detalles de Ventas del dia</h4>
			...
			//Inicio de tabla		
			$aux_salida.="<td valign='top'>
				   <h4>Totales por productos</h4>
					   <table class='table table-bordered table-hover table-striped table-condensed dataTable'>
							<thead>
								<tr>
									<td><b>Nombre</b></td>
...
//indice para para registros de detalle
			...
			$aux_salida.="
				...
				//indice para para registros de detalle
			$i=0;
				$total_sales[$i]['cant_sale']=$cant_ventas;	
				$total_sales[$i]['total_sale']=$acumlado;	
			//cierre de tabla gral
		   $aux_salida.="</tr></table>";
	   
	   }
	   //inserto en las tablas correspondientes el cierre
		$this->reports_model->addCloseDay($details_sales,$total_sales,$products_totals);
		$data['salida']=$aux_salida;
		$this->load->view('commons/header', $meta);
		$this->load->view('close_totals', $data);
		$this->load->view('commons/footer');
   
   }
  // totales de ventas semanales 
   
   function close_totals_weekly(){
   echo $this->input->get('user');
   
	$indice_dias= array('Domingo','Lunes','Martes','Miércoles','Jueves','Viernes','Sábado');
    // inicio de variables para locales 
	$total_productos=0;
	$cant_ventas=0;
	$acumlado=0;
	$aux_salida='';
	//$fecha= date('Y-m-').(date('d')-6);
	$indice_dia=date('w');	
	switch($indice_dia){
		case 0:
			$fecha=date('Y-m-').(date('d')-6);//domingo
			$fecha_texto=(date('d')-6).'/'.date('m/Y').' al domingo '.date('d/m/Y');//domingo
			break;
		case 1:
			$fecha=date('Y-m-').(date('d')-0);//lunes
			$fecha_texto=(date('d')-0).'/'.date('m/Y').' al lunes '.date('d/m/Y');//lunes
			break;
		case 2:
			$fecha=date('Y-m-').(date('d')-1);//martes
			$fecha_texto=(date('d')-1).'/'.date('m/Y').' al martes '.date('d/m/Y');//martes
			break;
		case 3:
			$fecha=date('Y-m-').(date('d')-2);//miercoles
			$fecha_texto=(date('d')-2).'/'.date('m/Y').' al miércoles '.date('d/m/Y');//miercoles
			break;
		case 4:
			$fecha=date('Y-m-').(date('d')-3);//jueves
			$fecha_texto=(date('d')-3).'/'.date('m/Y').' al jueves '.date('d/m/Y');//jueves
			break;
		case 5:
			$fecha=date('Y-m-').(date('d')-4);//viernes
			$fecha_texto=(date('d')-4).'/'.date('m-Y').' al viernes '.date('d/m/Y');//viernes
			break;
		case 6:
			$fecha=date('Y-m-').(date('d')-5);//sabado
			$fecha_texto=(date('d')-5).'/'.date('m/Y').' al sábado '.date('d/m/Y');//sabado
			break;
	}
	$texto_fecha="Desde el lunes ".$fecha_texto;
	// Recuperamos las ventas de la semana actual 
	$ventas=$this->reports_model->getTotalSalesDailyByDate($fecha);	
	 //Traemmos cantidad de productos vendidos
	$productos=$this->reports_model->getProductsSalesDailyByDate($fecha);
	if(!empty($ventas)){	
		// totales detallados de ventas
		$aux_salida="
			<h3>$texto_fecha</h3>		
		   <table width='100%'>
			...
			foreach($productos as $producto){
				$aux_salida.="
					<tr>
						<td>".$indice_dias[date("w", strtotime($producto->fecha))].' '.date("d/m", strtotime($producto->fecha))."</td>
						<td>".$producto->nombre."</td>
						<td>".number_format($producto->cantidad, 3, ',', '.')." Kg</td>
						<td>$ ".number_format($producto->total, 2, ',', '.')."</td>
					</tr>";	
			...
			$aux_salida.="	<td valign='top'>
					<h4>Detalles de ventas por día</h4>
					<table class='table table-bordered table-hover table-striped table-condensed dataTable'>
						<thead>
							<tr>
								<td><b>Fecha </b></td>
								<td><b>Cantidad de de ventas </b></td>
								<td><b>Total </b></td>
							</tr>
						</thead>";
			...
			foreach($ventas as $venta){
				$acumlado=$acumlado+$venta->total;
				$cant_ventas=$cant_ventas+$venta->cantidad;			
				$aux_salida.="
					<tr>
						<td>".$indice_dias[date("w", strtotime($venta->fecha))].' '.date("d/m", strtotime($venta->fecha))."</td>
						<td><i>".number_format($venta->cantidad, 0, ',', '.')."</i></td>
