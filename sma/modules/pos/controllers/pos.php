...
//Add new pos sale
   ...
   function index()
   {
		...
		{ 
			if($suspend) {
				...
				print_r($this->input->post('id_caja_turno'));
				if($saleID = $this->pos_model->addSale($saleDetails, $items, $warehouse_id, $did,$this->input->post('id_caja_turno'))) {
   ...
   /***
   ** scanea los cod de barrass segun si es un producto cargado o uno de la balanza
   **/
   function scan_product()
   {
		//if($this->input->get('code')){
			...
			//$code = $this->input->get('code');
		//}else{ //sino tiene cod este proviene de la balanza y se le devuelve el peso 
			/*modificado*/
			//$codigo=$code[2].$code[3].$code[4].$code[5];		   
			//$peso=$code[6].$code[7].$code[8].'.'.$code[9].$code[10].$code[11];
			/*fin*/	
			$code = $this->input->get('code');			
			if($this->pos_model->getCantCode($code)>0){
				$codigo=$code;
			}else{
				$codigo=$code[2].$code[3].$code[4].$code[5];
				$peso=$code[6].$code[7].$code[8].'.'.$code[9].$code[10].$code[11];
	  ...
	  $data['page_title'] = $this->lang->line("invoice");
	   ...
	   /***presonalizadas***/
		...
		function add_egresos(){
			...
			$data['id_caja_turno'] = $this->input->post('id_caja_turno');			
			$data['hora']=date("G:i:s");		
			$data['fecha']=date("Y-m-d");			
			if($this->pos_model->addCustomer($data))
				echo json_encode(array('confirmacion'=>'ok'));
	   
		}
		
		function check_turno_inicial(){
			$myQuery = "SELECT id_tipo,descripcion
			FROM tipo_egresos 
			GROUP BY id_tipo ASC";
		...
		function ini_caja(){
			$data['caja_inicial'] = $this->input->post('caja_inicial');
			$id_caja_turno=$this->pos_model->addInicio($data);	
			if($id_caja_turno)
				echo json_encode(array('id_caja_turno'=>$id_caja_turno));		
		}
		
		function get_rendicion_turno(){
			$id_caja_turno=$this->input->post('id_caja_turno');
			$totales['total_ventas_turno']=$this->total_ventas_turno($id_caja_turno);
			$totales['total_egresos_turno']=$this->total_egreso_turno($id_caja_turno);
			$caja_inicial=$this->caja_inicial($id_caja_turno);
			$totales['caja_inicial']=$caja_inicial[0]->caja_inicial;		
			echo json_encode($totales);			
		}
		
		function total_ventas_turno($id_caja_turno=null){
			$where="";
			if(isset($id_caja_turno) && $id_caja_turno != null)
				$where=" AND vt.id_caja_turno=$id_caja_turno";
			$myQuery = "SELECT 
							sum(s.total) as total
							FROM 
								ventas_x_turno vt, sales s 
							WHERE 
								vt.id_sales=s.id 
								$where";
			$q = $this->db->query($myQuery, false);
		...
		function total_egreso_turno($id_caja_turno=null){
			$where="";
			if(isset($id_caja_turno) && $id_caja_turno != null)
				$where=" AND ce.id_caja_turno=$id_caja_turno";
			$myQuery = "SELECT 
								sum(ce.monto) as total
							FROM 
								caja_egresos ce 
							WHERE 
								1=1 $where";
		...
		function caja_inicial($id_caja_turno=null){
			$where="";
			if(isset($id_caja_turno) && $id_caja_turno != null)
				$where=" AND c.id_caja_turno=$id_caja_turno";
			$myQuery = "SELECT 
								c.id_caja_turno
								,c.turno
								,c.caja_inicial
		...
		function total_retiro_turno(){
			
		}
		
		function cierre_turno(){
			//valores de cierre de turno			
			$data['id_turno'] = $this->input->post('id_caja_turno');			
			$data['hora']=date("G:i:s");		
			$data['fecha']=date("Y-m-d");
			$ventas_x_turno=$this->total_ventas_turno($data['id_turno']);
			$egreso_x_turno=$this->total_egreso_turno($data['id_turno']);			
			$data['total_ventas']=$ventas_x_turno[0]->total;
			$data['total_egresos']=$egreso_x_turno[0]->total;					
			$this->pos_model->cierreTurno($data);
		}
		
		function totalesDia(){
			$ventas_x_turno=$this->total_ventas_turno();
			$egreso_x_turno=$this->total_egreso_turno();			
		}
		
		function autocompletableProductos(){
			$term = $this->input->post('term');
			$where=" name LIKE '%$term%' OR code LIKE '%$term%' ";			
			if($where !=''){ 				
				$myQuery="SELECT 
					id
					,CONCAT(code, ' - ', name,' $ ',price) as codNomArticulo
				FROM 
					products
				WHERE
					$where
				...
				if($q->num_rows() > 0) {
					foreach (($q->result()) as $row) {
						$data[] = array($row->id=>$row->codNomArticulo);						
					}
					echo json_encode($data);
				}
			}
		...
		function autocompletableCodigo(){
