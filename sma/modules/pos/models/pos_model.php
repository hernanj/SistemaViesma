...
class Pos_model extends CI_Model
{
   ...
   public function categories_count() {
	...
	public function addSale($saleDetails = array(), $items = array(), $warehouse_id, $sid = NULL,$id_caja_turno=null)
	{
		...
		if($this->db->insert('sales', $saleData)) {
			...
			if($this->db->insert_batch('sale_items', $items)) {
				...
				$this->addVentaPorTurno($sale_id,$id_caja_turno);
				return $sale_id;
			}
		}
	...
	*/
	...
	public function addCustomer($data)
	{
		...
		if($this->db->insert('caja_egresos', $data)) {
			return true;
		}
		return false;
	}
	...
	public function getProductsByCodePrice(){
	
	
	}
	
	public function get_tipos_egresos(){
	...
	public function ingresoInicial(){	
		$myQuery = "SELECT count(*) as cantidad FROM sales WHERE date=DATE(now())";
		$q = $this->db->query($myQuery, false);
		if($q->num_rows() > 0) {
			foreach (($q->result()) as $row) {
				$data[] = $row;
	...
	public function hayTurno(){	
		$myQuery = "SELECT count(*) as cantidad FROM caja_turnos WHERE fecha_turno=DATE(now()) AND estado !='IN'";
		$q = $this->db->query($myQuery, false);
		if($q->num_rows() > 0) {
			foreach (($q->result()) as $row) {
				$data[] = $row;
	...
	public function numeroTurno(){	
		$myQuery = "SELECT count(*) as cantidad FROM caja_turnos WHERE fecha_turno=DATE(now()) ";
		$q = $this->db->query($myQuery, false);
		if($q->num_rows() > 0) {
			foreach (($q->result()) as $row) {
				$data[] = $row;
	...
	public function addInicio($datos=array()){
		$datos['fecha_turno']=date("Y-m-d");
		$datos['hora_turno']=date("G:i:s");		
		if($this->db->insert('caja_turnos', $datos)) {
	...
	public function cierreTurno($datos=array()){
		 	/*
			$cierre['id_turno']=$datos['id_turno'];
			$cierre['fecha']=$datos['id_turno'];
			$cierre['hora']=;
			$cierre['total_ventas']=;
			$cierre['total_egresos']=;
			$cierre['sobrante_truno']=0;
			$cierre['faltante_turno']=0;
			$cierre['adelanto_efectivo']=0;
			$cierre['retiro_efectivo']=0;
			$cierre['adelanto_mercaderia']=0;
			$cierre['caja_proximo_turno']=0;
			$cierre['rendicion_efectivo']=0;*/
		// insert con el cierre del turno
		if($this->db->insert('cierre_turno', $datos)) {
			$id[]=$this->db->insert_id();
		}
		//update 
		if($this->db->update('caja_turnos', array('estado'=>'IN'),array('id_caja_turno'=>$datos['id_turno']))){
				$id[]= $this->db->insert_id();
		}
		if(!empty($id))		
			return true;
	...
	public function addVentaPorTurno($id_sale='',$id_caja_turno=null){	
		$datos['id_sales']=$id_sale;
		$datos['hora']=date("G:i:s");
		$datos['id_caja_turno']=$id_caja_turno;
		//A definir
		//$datos['id_tipo_venta']=;
		if($this->db->insert('ventas_x_turno', $datos)) {
			return true;
		}
		return false;		
	}
	
	public function getIdCajaTurnoActual(){
		$myQuery = "SELECT * FROM caja_turnos WHERE fecha_turno=DATE(now()) AND estado = 'AC'";
		$q = $this->db->query($myQuery, false);
		if($q->num_rows() > 0) {
			foreach (($q->result()) as $row) {
				$data[] = $row;
	...
	public function getCantCode($code=''){
		$myQuery = "SELECT * FROM products WHERE  code = '$code'";
		$q = $this->db->query($myQuery, false);
		return $q->num_rows();	
	}
